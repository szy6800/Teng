...
class WjSpider(scrapy.Spider):
    ...
    start_urls = ['http://swj.shenyang.gov.cn/sysswj/tzgg/glist1.html']
    # start_urls = ['http://swj.shenyang.gov.cn/sysswj/tzgg/glist{}.html'.format(i) for i in range(1,25)]
    ...
    def parse(self, response):
        ...
        list_url = response.xpath('//*[@class="list-wrap1"]/li/a[1]/@href').getall()
        pub_dates = response.xpath('//*[@class="time"]/text()').getall()
        # print(pub_dates)
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        # 标题
        item['title'] = response.xpath("//*[@class='cent-bt']/text()").get().strip()
        # 城市id 1117
        item['cityId'] = 1117
        ...
        content = html.xpath('//*[@class="cent-center"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="cent-center"]')
        ...
        compal = re.compile('style=".*?"|http://swj.shenyang.gov.cn/saas/plugins/ueditor/dialogs/attachment/fileTypeImages/icon_.*?gif|face=".*?"')
        contentHtml = re.sub(compal, '', contentHtml)
        try:
            tables = html.xpath('//*[@class="cent-center"]//table/@width')[0]
