...
class WjSpider(scrapy.Spider):
    ...
    start_urls = ['http://czj.shenyang.gov.cn/syczj/zwgkzdgz/tzgg/glist.html']
    # start_urls = ['http://czj.shenyang.gov.cn/syczj/zwgkzdgz/tzgg/glist{}.html'.format(i) for i in range(1,34)]
    ...
    def parse(self, response):
        ...
        list_url = response.xpath('//*[@class="lb"]/a[1]/@href').getall()
    ...
    def parse_info(self, response):
        ...
        # 标题
        item['title'] = response.xpath("//*[@class='btk']/text()").get().strip()
        # 城市id 1117
        item['cityId'] = 1117
        ...
        content = html.xpath('//*[@class="nr"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="nr"]')
        ...
        compal = re.compile('style=".*?"|http://czj.shenyang.gov.cn/saas/plugins/ueditor/dialogs/attachment/fileTypeImages/icon_.*?gif|face=".*?"')
        contentHtml = re.sub(compal, '', contentHtml)
        try:
            tables = html.xpath('//*[@class="nr"]//table/@width')[0]
