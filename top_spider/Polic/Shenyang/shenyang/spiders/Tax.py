...
# http://liaoning.chinatax.gov.cn/col/col315/index.html 通知公告

class WjSpider(scrapy.Spider):
    name = 'Tax'
    allowed_domains = ['liaoning.chinatax.gov.cn']
    # start_urls = ['http://liaoning.chinatax.gov.cn/']

    def start_requests(self):
        # 构建url
        #for l1, l2 in zip(range(1, 550, 45), range(45, 550, 45)):
        for l1, l2 in zip(range(1, 45, 45), range(45, 90, 45)):
            url = 'http://liaoning.chinatax.gov.cn/module/web/jpage/dataproxy.jsp?startrecord={}&endrecord={}&perpage=15'.format(
                l1, l2)
            # 构建post请求参数
            data = {
                'col': '1',
    ...
    def parse_info(self, response):
        ...
        compal = re.compile('style=".*?"|http://liaoning.chinatax.gov.cn/module/jslib/icons/.*?png')
