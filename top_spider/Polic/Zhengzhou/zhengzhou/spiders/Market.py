...
class WjSpider(scrapy.Spider):
    name = 'Market'
    allowed_domains = ['amr.zhengzhou.gov.cn']
    start_urls = ['http://amr.zhengzhou.gov.cn/gsgg/index.jhtml'] #25
    # start_urls = ['http://amr.zhengzhou.gov.cn/wjfb/index.jhtml']
    ...
    def parse(self, response):
        item = {}
        # # 反序列化
        list_url = response.xpath('//*[@class="box-content"]//ul[@class="news-list date-right"]/li/a/@href').getall()
        pub_dates = response.xpath('//*[@class="box-content"]//ul[@class="news-list date-right"]/li/a/@date-time').getall()
        titles = response.xpath('//*[@class="box-content"]//ul[@class="news-list date-right"]/li/a/text()').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[@class="news_content_content"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="news_content_content"]')
        ...
        try:
            tables = html.xpath('//*[@class="news_content_content"]//table/@width')[0]
