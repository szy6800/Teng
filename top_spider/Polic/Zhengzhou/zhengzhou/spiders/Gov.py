...
#  http://public.zhengzhou.gov.cn/?a=theme&n=3&page=5

class WjSpider(scrapy.Spider):
    name = 'Gov'
    allowed_domains = ['zhengzhou.gov.cn']
    # start_urls = ['http://zzcz.zhengzhou.gov.cn/ggtz/index_{}.jhtml'.format(i) for i in range(2,34)] #34
    start_urls = ['http://public.zhengzhou.gov.cn/?a=dir&page={}'.format(i) for i in range(1,2)] #34
    ...
    #             p = f"_{p+1}" if p else ""
    #             url = f"http://zzrs.zhengzhou.gov.cn/{cate}{p}.jhtml"
    ...
    def parse(self, response):
        ...
        list_url = response.xpath('//*[@class="page-list"]/a/@href').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        # 标题
        item['title'] = response.xpath("//*[@class='content-title']/text()").get().strip()
        # 城市id 153
        item['cityId'] = 153
        ...
        content = html.xpath('//*[@class="content-txt"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="content-txt"]')
        ...
        for p in p_list:
            ...
            try:
                ...
                pass
        # # 将新的正文标签转成str保存
        contentHtml = etree.tostring(div_data[0], encoding='utf-8').decode().replace('扫一扫在手机打开当前页', '')
        compal = re.compile('style=".*?"|http://amr.ah.gov.cn/assets/images/files2/.*?gif|face=".*?"')
        contentHtml = re.sub(compal, '', contentHtml)
        try:
            tables = html.xpath('//*[@class="content-txt"]//table/@width|//*[@class="content-txt"]//table/@width')[0]
            item['contentHtml'] = re.sub(tables, '', contentHtml)
        except:
            item['contentHtml'] = re.sub(compal, '', contentHtml)
        try:
            pub_date = response.xpath("//*[contains(@title,'发布日期')]/following::em[1]/text()").get()
