...
# http://jrgzj.xa.gov.cn/tzzx/6.html


class WjSpider(scrapy.Spider):
    name = 'Banking'
    allowed_domains = ['xa.gov.cn']
    start_urls = ['http://jrgzj.xa.gov.cn/tzzx/{}.html'.format(i) for i in range(1,2)]  #6
    ...
    def parse(self, response):
        ...
        list_url = response.xpath('//*[@id="lst-rt"]/article/a[1]/@href').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        pub_date = response.xpath('//*[@class="m-txt-crm hidden-sm hidden-xs"]//span[contains(text(),"发布时间：")]').get()
        timeArray = re.findall('\d{4}-\d{2}-\d{2}',pub_date)[0]
        timeArray = time.strptime(timeArray, "%Y-%m-%d")
        timeStamp = int(time.mktime(timeArray)) * 1000
        item['publishTime'] = timeStamp
        # 新闻类型
        item['typeId'] = type_polic(item)
        # 来源
        item['sourceSite'] = '西安市金融监管局'
        # 行业
        item['industry'] = ''
        # 行业id
        item['industryId'] = ''
