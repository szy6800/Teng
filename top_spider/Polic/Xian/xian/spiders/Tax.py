...
# 最新文件 https://shaanxi.chinatax.gov.cn/col/col7526/index.html

class WjSpider(scrapy.Spider):
    name = 'Tax'
    allowed_domains = ['shaanxi.chinatax.gov.cn']
    # start_urls = ['http://shaanxi.chinatax.gov.cn/']

    def start_requests(self):
        # 构建url
        # for l1, l2 in zip(range(1, 475, 45), range(45, 500, 45)):
        for l1, l2 in zip(range(1, 45, 45), range(45, 90, 45)):
            url = 'https://shaanxi.chinatax.gov.cn/module/web/jpage/dataproxy.jsp?startrecord={}&endrecord={}&perpage=15'.format(
                l1, l2)
            # 构建post请求参数
            data = {
                'col': '1',
                'webid': '2',
                'columnid': '7526',
                'path': 'https: // shaanxi.chinatax.gov.cn /',
                'sourceContentType': '1',
                'unitid': '26674',
                'webname': '国家税务总局陕西省税务局',
                'permissiontype': '0'
            }
            # 发送post请求
    ...
    def parse(self, response):
        ...
        for href in list_url:
            item['url'] = response.urljoin(href).replace('module/web/jpage/ / shaanxi.chinatax.gov.cn /', '')
            # print(item['url'])
    ...
    def parse_info(self, response):
        ...
        compal = re.compile('style=".*?"|https://shaanxi.chinatax.gov.cn/module/jslib/icons/.*?png')
