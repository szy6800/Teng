...
# http://shanxi.chinatax.gov.cn/son/list/ty-11401-522


class GovSpider(scrapy.Spider):
    name = 'Tax'
    allowed_domains = ['taiyuan.gov.cn']
    start_urls = ['http://shanxi.chinatax.gov.cn/common/extQuery?sqlid=web_data_wz2&limit=15&lmdm=522&orgid=11401&ptwz=N&page={}'.format(i) for i in range(1,2)]

    def parse(self, response):
        # print(response.text)
        item = {}
        text = json.loads(response.text)
        list_url = jsonpath.jsonpath(text,'$..WZID')
        # 循环遍历
        for href in list_url:
            # print(response.urljoin(href))
            item['url'] = 'http://shanxi.chinatax.gov.cn/son/detail/ty-11401-522-' + str(href)
            # print(item['url'])
