...
class WjSpider(scrapy.Spider):
    ...
    start_urls = ['http://27.223.1.61:8090/GetDynamicPager.ashx?showtotal=1&templateGuid=201020153045204040&lkocok_pageNo={}&htmlPageCount=30&page=changeInfo'.format(i) for i in range(1,2)]
    ...
    def parse(self, response):
        ...
        pub_dates = re.findall("<span>(.*?)</span>",response.text)
        # 循环遍历
        for href, pub_date in zip(list_url, pub_dates):
            item['url'] = 'http://qdcz.qingdao.gov.cn/' + href
    ...
    def parse_info(self, response):
        ...
        # 标题
        item['title'] = response.xpath("//*[@class='content_title']//text()").get().strip()
        # 城市id
        item['cityId'] = 5
        ...
        content = html.xpath('//*[@class="content_wz"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="content_wz"]')
        ...
        try:
            tables = html.xpath('//*[@class="content_wz"]//table/@width')[0]
