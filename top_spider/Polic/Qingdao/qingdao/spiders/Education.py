...
# 青岛市 http://edu.qingdao.gov.cn/n32561912/n32561915/index.html
...
class WjSpider(scrapy.Spider):
    ...
    start_urls = ['http://edu.qingdao.gov.cn/n32561912/n32561915/index.html']
    ...
    def parse(self, response):
        ...
        # print(list_url)
        pub_dates = response.xpath('//*[@class="span"]//text()').getall()
        # 循环遍历
        for href, pub_date in zip(list_url, pub_dates):
            item['url'] = 'http://edu.qingdao.gov.cn' + href
    ...
    def parse_info(self, response):
        ...
        # 标题
        item['title'] = response.xpath("//*[@class='xiangxi']//text()").get().strip()
        # 城市id
        item['cityId'] = 1211
        ...
        content = html.xpath('//*[@class="xiangxi_wz"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="xiangxi_wz"]')
        ...
        try:
            tables = html.xpath('//*[@class="xiangxi_wz"]//table/@width')[0]
