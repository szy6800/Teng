...
# 成都市商务局 http://sww.chengdu.gov.cn/cdswh/dwmy/gdlist1526618933789.shtml

class WjSpider(scrapy.Spider):
    name = 'Commerce'
    allowed_domains = ['chengdu.gov.cn']

    start_urls = ['http://sww.chengdu.gov.cn/es-search/search/c90fd69b76c94f45950d83f0d463cc30?_template=ucap/list_ejs&_pageSize=15&_isAgg=1&page={}'.format(i) for i in range(1,2)] #对外贸易  14
    #
    ...
    def parse(self, response):
        ...
        list_url = response.xpath('//*[@class="list-li"]//a[1]/@href').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[@id="detail"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@id="detail"]')
        ...
        compal = re.compile('style=".*?"|http://sww.chengdu.gov.cn/default/images/icon16/.*?gif|http://cdjx.chengdu.gov.cn/cdsjxw/uploadfiles/.*?gif')

        contentHtml = re.sub(compal, '', contentHtml)
        try:
            tables = html.xpath('//*[@id="detail"]//table/@width')[0]
