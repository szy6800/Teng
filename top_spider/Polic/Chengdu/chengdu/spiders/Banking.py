...
import copy
from .Banking_text import type_polic,index

# 成都市金融监管局 http://jr.chengdu.gov.cn/jinrongban/c139004/zcfg1_2.shtml

class WjSpider(scrapy.Spider):
    name = 'Banking'
    allowed_domains = ['chengdu.gov.cn']

    # start_urls = ['http://jr.chengdu.gov.cn/jinrongban/c139004/zcfg1_2.shtml'.format(i) for i in range(1,2)] #对外贸易  14
    #
    def start_requests(self):
        for each in index():
            ...
            for p in range(pages):
                p = f"_{p + 1}" if p else ""
                url = f"http://jr.chengdu.gov.cn/{cate}{p}.shtml"
                yield scrapy.Request(url=url, callback=self.parse, dont_filter=True)

    def parse(self, response):
        item = {}
        # text = json.loads(response.text)
        list_url = response.xpath('//*[@class="lh-policy-item"]//a[1]/@href|//*[@class="lh-notice-item"]//a[1]/@href').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[@class="lh-news-info clearfix"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="lh-news-info clearfix"]')
        ...
        compal = re.compile('style=".*?"|http://sww.chengdu.gov.cn/default/images/icon16/.*?gif|face=".*?"')
        contentHtml = re.sub(compal, '', contentHtml)
        try:
            tables = html.xpath('//*[@class="lh-news-info clearfix"]//table/@width')[0]
