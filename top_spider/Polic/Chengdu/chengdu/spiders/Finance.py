...
# TODO  成都财政局 http://cdcz.chengdu.gov.cn/cdsczj/c116712/jgsz_list.shtml
class Wj4Spider(scrapy.Spider):
    name = 'Finance'
    allowed_domains = ['chengdu.gov.cn']
    # start_urls = ['http://cdcz.chengdu.gov.cn/cdsczj/c116712/jgsz_list_{}.shtml'.format(i) for i in range(2,15)]
    # start_urls = ['http://cdcz.chengdu.gov.cn/cdsczj/c116712/jgsz_list.shtml']
    ...
    def start_requests(self):
        for each in index():
            ...
            for p in range(pages):
                p = f"_{p+1}" if p else ""
                url = f"http://cdcz.chengdu.gov.cn/{cate}{p}.shtml"
                yield scrapy.Request(url=url, callback=self.parse, dont_filter=True)

    def parse(self, response):
        item = {}
        # text = json.loads(response.text)
        list_url = response.xpath('//*[@class="gzdt_ul"]/li/a[1]/@href|//*[@class="bsfw_ul"]/li/a[1]/@href').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[@class="NewsContent"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="NewsContent"]')
        ...
        compal = re.compile('style=".*?"|http://cdcz.chengdu.gov.cn/default/images/icon16/.*?gif|face=".*?"')
        contentHtml = re.sub(compal, '', contentHtml)
        try:
            tables = html.xpath('//*[@class="NewsContent"]//table/@width')[0]
