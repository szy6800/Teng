...
class WjSpider(scrapy.Spider):
    ...
    # start_urls = ['http://stic.sz.gov.cn/xxgk/tzgg/index_2.html'] #40
    start_urls = ['http://search.gd.gov.cn/jsonp/site/755001?callback=jQuery1910736171581993665_1626256638068&order=1&category_id=2279&page={}&including_url_doc=1&including_link_doc=1&including_attach_doc=1&pagesize=10&json_ext_terms=%7B%22EXT_jdfs%22%3A%5B%22%E6%96%87%E5%AD%97%22%2C%221%22%5D%7D&_=1626256638073'.format(i) for i in range(1,2)] #政策解读 90
    ...
    def parse(self, response):
        # print(response.text)
        item = {}
        text = response.text.replace('jQuery1910736171581993665_1626256638068(', '').replace(')', '')
        text = json.loads(text)
        # print(text)
        list_url = jsonpath.jsonpath(text, '$..post_url')
        # print(list_url)
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        for elem in html.xpath('//style|//script|//*[@class="bshare-custom icon-medium"]|//*[@class="print"]|//*[@id="Canvas"]'):
            elem.getparent().remove(elem)
        content = html.xpath('//*[@class="news_cont_d_wrap"]//text()|//*[@class="article-content"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="news_cont_d_wrap"]|//*[@class="article-content"]')
        ...
        try:
            tables = html.xpath('//*[@class="news_cont_d_wrap"]//table/@width|//*[@class="article-content"]//table/@width')[0]
