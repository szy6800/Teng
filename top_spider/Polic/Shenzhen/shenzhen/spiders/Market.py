...
class Pub2Spider(scrapy.Spider):
    name = 'Market'
    allowed_domains = ['amr.sz.gov.cn']
    ...
    def start_requests(self):
        for each in index():
            ...
            for p in range(pages):
                p = f"_{p + 1}" if p else ""
                url = f"http://amr.sz.gov.cn/{cate}index{p}.html"
                # print(url)
    ...
    def parse(self, response):
        item = {}
        # # 反序列化
        list_url = response.xpath('//*[@class="newslist"]/ul/li/a/@href|//*[@id="post-list"]/tr//td/a/@href').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[@class="articlecontent"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="articlecontent"]')
