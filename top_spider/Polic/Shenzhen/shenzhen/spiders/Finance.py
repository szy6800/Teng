...
class WjSpider(scrapy.Spider):
    name = 'Finance'
    allowed_domains = ['szfb.sz.gov.cn']
    #20
    # start_urls = ['http://jr.sz.gov.cn/sjrb/xxgk/tzgg/index_{}.html'.format(i) for i in range(2,46)]
    start_urls = ['http://search.gd.gov.cn/jsonp/site/755021?callback=jQuery34105711567340668142_1618293794377&order=1&category_id=24801&page={}&pagesize=20&including_url_doc=1&including_link_doc=1&_=1618293794380'.format(i) for i in range(1,2)] #40
    ...
    def parse(self, response):
        # print(response.text)
        item = {}
        text = response.text.replace('jQuery34105711567340668142_1618293794377(','').replace(')','')
