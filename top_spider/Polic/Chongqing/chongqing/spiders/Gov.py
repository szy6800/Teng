...
# 重庆市 http://czj.cq.gov.cn/zwgk_268/gsgg/ywgg/index_1.html
class WjSpider(scrapy.Spider):
    name = 'Gov'
    allowed_domains = ['cq.gov.cn']
    # start_urls = ['http://www.cq.gov.cn/zwgk/zfxxgkml/zcjd_120614/bmjd/index_{}.html'.format(i) for i in range(2, 26)]
    start_urls = ['http://www.cq.gov.cn/zwgk/zfxxgkml/zcjd_120614/bmjd/index.html']
    ...
    def parse(self, response):
        ...
        list_url = response.xpath('//*[@class="common-list"]//li//a/@href').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        for elem in html.xpath('//style|//script|//*[@id="div_div"]|//*[@class="print"]|//*[@id="Canvas"]|//*[@class="bdsharebuttonbox bdshare-button-style1-24"]|//*[@class="bdsharebuttonbox"]'):
            elem.getparent().remove(elem)
        content = html.xpath('//*[@class="c-txt left"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="c-txt left"]')
        ...
        compal = re.compile('style=".*?"|http://jrjgj.cq.gov.cn/govapp/lib/ueditor_demo/ueditor2/dialogs/attachment/fileTypeImages/.*?gif|face=".*?"')
        contentHtml = re.sub(compal, '', contentHtml)
        try:
            tables = html.xpath('//*[@class="c-txt left"]//table/@width')[0]
