...
class WjSpider(scrapy.Spider):
    name = 'Tax'
    allowed_domains = ['chongqing.chinatax.gov.cn']
    # start_urls = ['http://chongqing.chinatax.gov.cn/']
    ...
    def start_requests(self):
        for each in index():
            ...
            for p in range(pages):
                p = f"_{p}" if p else ""
                url = f"http://chongqing.chinatax.gov.cn/{cate}{p}.html"
                yield scrapy.Request(url=url, callback=self.parse, dont_filter=True)

    def parse(self, response):
        item = {}
        # # 反序列化
        list_url = response.xpath('//*[@class="hot_news"]//a/@href').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[@class="details_main"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="details_main"]')
        ...
        for p in p_list:
            ...
            try:
                ...
                pass
        # # 将新的正文标签转成str保存
        contentHtml = etree.tostring(div_data[0], encoding='utf-8').decode().replace('扫一扫，在手机打开当前页面', '')
