...
from .xa_text import index, type_polic

class TongMakeSpider(scrapy.Spider):
    name = 'type1'
    ...
    def start_requests(self):
        for each in index():
            ...
            for p in range(pages):
                p = f"{p+1}" if p else ""
                url = f"http://search.xiongan.gov.cn/{cate}{p}"
                yield scrapy.Request(url=url, callback=self.parse)

    def parse(self, response):
        text = json.loads(response.text)
        item = {}
        list_url = jsonpath.jsonpath(text, '$..originUrl')
        PubTime = jsonpath.jsonpath(text, '$..releaseDate')
        Titles = jsonpath.jsonpath(text, '$..title')
        SourceNames = jsonpath.jsonpath(text, '$..siteName')
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[@class="main-content-box"]//text()|//*[@class="main"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
