...
# http://www.dg.gov.cn/gkmlpt/index#681
class WjSpider(scrapy.Spider):
    name = 'Gov'
    allowed_domains = ['dg.gov.cn']
    # start_urls = ['http://czj.dg.gov.cn/flfgjgfxwj/index_{}.html'.format(i) for i in range(2,11)]
    start_urls = ['http://www.dg.gov.cn/gkmlpt/api/all/681?page={}&sid=769001'.format(i) for i in range(1,2)]
    ...
    def parse(self, response):
        ...
        # list_url = response.xpath('//*[@class="list-right_title fon_1"]/a/@href').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[@class="article-content"]//text()|//*[@id="zoomcon"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="article-content"]|//*[@id="zoomcon"]')
        ...
        compal = re.compile('style=".*?"|http://dghrss.dg.gov.cn/default/images/icon16/.*?gif|face=".*?"')
        contentHtml = re.sub(compal, '', contentHtml)
        try:
            tables = html.xpath('//*[@class="article-content"]//table/@width')[0]
