...
class WjSpider(scrapy.Spider):
    name = 'Tax'
    allowed_domains = ['guangdong.chinatax.gov.cn']

    # start_urls = ['https://guangdong.chinatax.gov.cn/gdsw/dgsw_tzgg/city_list_{}.shtml'.format(i)  for i in range(70,90)] #通知公告 90
    start_urls = ['https://guangdong.chinatax.gov.cn/gdsw/dgsw_jcgg/city_list.shtml','https://guangdong.chinatax.gov.cn/gdsw/dgsw_tzgg/city_list.shtml'] #基层公告 73
    ...
    #             p = f"{p + 1}" if p else ""
    #             url = f"http://hunan.chinatax.gov.cn/{cate}{p}"
    ...
    def parse_info(self, response):
        ...
        compal = re.compile('style=".*?"|https://guangdong.chinatax.gov.cn/gdsw/dgsw_tzgg/2021-01/12/7f524358090943b99424e26796c5ddfb/images/.*?gif|face=".*?"|https://guangdong.chinatax.gov.cn/app-editor/ewebeditor/sysimage/icon16/.*?gif')
        contentHtml = re.sub(compal, '', contentHtml)
        try:
            tables = html.xpath('//*[@class="content"]//table/@width')[0]
            item['contentHtml'] = re.sub(tables, '', contentHtml)
        except:
            item['contentHtml'] = re.sub(compal, '', contentHtml)

        pub_date = response.xpath("//publishtime/text()").get()
