...
from .Tax_text import *

class WjSpider(scrapy.Spider):
    name = 'Tax'
    allowed_domains = ['hubei.chinatax.gov.cn']
    # start_urls = ['http://hubei.chinatax.gov.cn/hbsw/wuhan/tzgg/sjtzgg/index9.html']
    # start_urls = ['http://hubei.chinatax.gov.cn/hbsw/xxgk/jyta/index.html']
    # start_urls = ['http://hubei.chinatax.gov.cn/hbsw/xxgk/cjgg/index.html']
    ...
    def start_requests(self):
        for each in index():
            ...
            for p in range(pages):
                p = f"{p}" if p else ""
                url = f"http://hubei.chinatax.gov.cn/{cate}{p}.html"
                # print(url)
    ...
    def parse(self, response):
        item = {}
        # # 反序列化
        list_url = response.xpath('//*[@class="newsList"]//li/a/@href').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        # 标题
        item['title'] = response.xpath("//*[@name='ArticleTitle']/@content").get().replace('<br/>', '')
        # 城市id
        item['cityId'] = 845
        ...
        content = html.xpath('//*[@class="dynamic-detail__content"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除空格
        ...
        div_data = html.xpath('//*[@class="dynamic-detail__content"]')
        ...
        compal = re.compile('style=".*?"|http://hubei.chinatax.gov.cn/cms/js/UEditor/dialogs/attachment/fileTypeImages/.*?gif')
        contentHtml = re.sub(compal, '', contentHtml)
        try:
            tables = html.xpath('//*[@class="dynamic-detail__content"]//table/@width')[0]
