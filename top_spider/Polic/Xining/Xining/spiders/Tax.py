...
# http://qinghai.chinatax.gov.cn/xns/sjtzgg/list_2.shtml


class GovSpider(scrapy.Spider):
    name = 'Tax'
    allowed_domains = ['chinatax.gov.cn']
    start_urls = ['http://qinghai.chinatax.gov.cn/xns/sjtzgg/list.shtml']

    def parse(self, response):
        # print(response.text)
        item = {}
        list_url = response.xpath('//*[@id="wzList"]//a/@href').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        for elem in html.xpath('//style|//script|//*[@id="div_div"]|//*[@class="ewm"]|//*[@id="Canvas"]'):
            elem.getparent().remove(elem)
        content = html.xpath('//*[@id="main"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@id="main"]')
        ...
        try:
            tables = html.xpath('//*[@id="main"]//table/@width')[0]
