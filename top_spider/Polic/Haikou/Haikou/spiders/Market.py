...
from .msBureau import indexMarket, type_polic, method_name, methodNameUrl
...
class MarketSpider(scrapy.Spider):
    ...
    # start_urls = ['http://amr.haikou.gov.cn/xxgk/gsgg/index.html']

    def start_requests(self):
        for each in indexMarket():
            cate = each["cate"]
            pages = each["pages"]
            for p in range(pages):
                p = f"_{p}" if p else ""
                url = f"http://amr.haikou.gov.cn/{cate}index{p}.html"
