...
import htmlmin
 ...
 # 税务局  http://shandong.chinatax.gov.cn/col/col1053/index.html?number=A0301&uid=17961&pageNum=41


class TaxbureauSpider(scrapy.Spider):
    name = 'Tax'
    allowed_domains = ['shandong.chinatax.gov.cn']

    def start_requests(self):
        # 构建url
        for l1, l2 in zip(range(1, 45, 45), range(45, 90, 45)):#650
            url = 'http://shandong.chinatax.gov.cn/module/web/jpage/dataproxy.jsp?startrecord={}&endrecord={}&perpage=15'.format(
                l1, l2)
            # 构建post请求参数
            data = {
                'col': '1',
