...
import copy
from .Economics_text import type_polic,index
import json ,jsonpath

#  http://jxj.beijing.gov.cn/zwgk/flfg/gjflfggz/bmgz/ 北京市经济和信息化局

class WjSpider(scrapy.Spider):
    name = 'Economics'
    allowed_domains = ['jxj.beijing.gov.cn']
    # start_urls = ['http://jxj.beijing.gov.cn/zwgk/zcwj/gjzc/index_1.html']
    ...
    def start_requests(self):
        for each in index():
            ...
            for p in range(pages):
                p = f"_{p}" if p else ""
                url = f"http://jxj.beijing.gov.cn/{cate}{p}.html"
