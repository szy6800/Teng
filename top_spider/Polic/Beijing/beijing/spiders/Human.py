...
import copy
from .Human_text import type_polic,index
import json ,jsonpath

#  http://rsj.beijing.gov.cn/xxgk/zcjd/ 人力资源局

class WjSpider(scrapy.Spider):
    name = 'Human'
    allowed_domains = ['rsj.beijing.gov.cn']
    # start_urls = ['http://rsj.beijing.gov.cn/xxgk/zcwj/']
    ...
    def start_requests(self):
        for each in index():
            ...
            for p in range(pages):
                p = f"_{p}" if p else ""
                url = f"http://rsj.beijing.gov.cn/{cate}{p}.html"
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[@id="mainTextZoom"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@id="mainTextZoom"]')
        ...
        try:
            tables = html.xpath('//*[@id="mainTextZoom"]//table/@width')[0]
