...
#  http://sw.beijing.gov.cn/zwxx/zcfg/gfxwj/ 商务局

class WjSpider(scrapy.Spider):
    name = 'Commerce'
    allowed_domains = ['sw.beijing.gov.cn']
    # start_urls = ['http://sw.beijing.gov.cn/zwxx/tzgg/index_1.html']
    ...
    def start_requests(self):
        for each in index():
            ...
            for p in range(pages):
                p = f"_{p}" if p else ""
                url = f"http://sw.beijing.gov.cn/{cate}{p}.html"
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[@class="detail"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="detail"]')
        ...
        try:
            tables = html.xpath('//*[@class="detail"]//table/@width')[0]
