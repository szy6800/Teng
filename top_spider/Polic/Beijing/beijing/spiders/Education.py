...
import copy
from .Education_text import type_polic,index

# 北京教育局 http://kw.beijing.gov.cn/
...
class WjSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for each in index():
            ...
            for p in range(pages):
                p = f"_{p}" if p else ""
                url = f"http://jw.beijing.gov.cn/{cate}index{p}.html"
    ...
    def parse(self, response):
        ...
        list_url = response.xpath('//*[@class="col-md"]/a/@href').getall()
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[contains(@class,"mycontont")]//text()')
        ...
        div_data = html.xpath('//*[contains(@class,"mycontont")]')
        ...
        compal = re.compile('style=".*?"|http://fwxt.czj.beijing.gov.cn/bjlaw/images/other/.*?gif|face=".*?"')
        contentHtml = re.sub(compal, '', contentHtml)
        try:
            tables = html.xpath('//*[contains(@class,"mycontont")]//table/@width')[0]
        ...
        attachment_url = response.xpath(
            '//*[(contains(translate(@href, "PDF", "pdf"), ".pdf") or contains(translate(@href, "XLS", "xls"), ".xls") or contains(translate(@href, "DOC", "doc"), ".doc") or contains(translate(@href, "ZIP", "zip"), ".zip") or contains(translate(@href, "RAR", "rar"), ".rar") or contains(translate(@href, "WPS", "wps"), ".wps")) and not(contains(@href, "file://") or contains(@href, "c:\") or contains(@href, "c:\"))]/@href | .//*[contains(translate(@src, "MP4","mp4"), ".mp4") and not(contains(@src6, "file://") or contains(@src,"c:\") or contains(@src, "C:\"))]/@src').getall()
        # 整合name和url
