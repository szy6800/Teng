...
import copy
from .Gov_text import type_polic,index

# 北京市政府 http://kw.beijing.gov.cn/

class WjSpider(scrapy.Spider):
    name = 'Gov'
    allowed_domains = ['czj.beijing.gov.cn']
    # start_urls = ['http://www.beijing.gov.cn/zhengce/zfwj/zfwj2016/bgtwj/index_1.html']
    def start_requests(self):
        for each in index():
            ...
            for p in range(pages):
                p = f"_{p}" if p else ""
                url = f"http://www.beijing.gov.cn/{cate}index{p}.html"
    ...
    def parse_info(self, response):
        ...
        pub_dates = response.xpath("//li[contains(text(),'[发布日期] ')]").get()
        pub_date = re.findall('\d{4}-\d{2}-\d{2}',pub_dates)[0]
