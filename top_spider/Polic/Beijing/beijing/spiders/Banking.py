...
import copy
from .Commerce_text import type_polic,index
import json ,jsonpath

#  http://jrj.beijing.gov.cn/tztg/index.html 金融

class WjSpider(scrapy.Spider):
    name = 'Banking'
    allowed_domains = ['jrj.beijing.gov.cn']
    # start_urls = ['http://jrj.beijing.gov.cn/tztg/index_{}.html'.format(i) for i in range(1,21)]
    start_urls = ['http://jrj.beijing.gov.cn/tztg/index.html']

    def parse(self, response):
        # print(response.text)
        item = {}
        # text = json.loads(response.text)
        list_url = response.xpath('//*[@class="dancon-list"]//li//a/@href').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[@class="zhengwen"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        ...
        try:
            tables = html.xpath('//*[@class="zhengwen"]//table/@width')[0]
