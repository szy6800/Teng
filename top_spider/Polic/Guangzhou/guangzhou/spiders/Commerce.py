...
class WjSpider(scrapy.Spider):
    ...
    name = 'Commerce'
    allowed_domains = ['sw.gz.gov.cn']
    # start_urls = ['http://sw.gz.gov.cn/xxgk/tzgg/tz/index_{}.html'.format(i) for i in range(2,3)] #20
    ...
    def start_requests(self):
        for each in index():
            ...
            for p in range(pages):
                p = f"_{p+1}" if p else ""
                url = f"http://sw.gz.gov.cn/{cate}{p}.html"
