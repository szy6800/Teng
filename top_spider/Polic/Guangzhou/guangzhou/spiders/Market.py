...
class Wj1Spider(scrapy.Spider):
    name = 'Market'
    allowed_domains = ['scjgj.gz.gov.cn','gz.gov.cn']
    ...
    def start_requests(self):
        for each in index():
            ...
            for p in range(pages):
                p = f"_{p + 1}" if p else ""
                url = f"http://scjgj.gz.gov.cn/{cate}{p}.html"
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[@class="articlecontent"]//text()|//*[@id="zoomcon"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="articlecontent"]|//*[@id="zoomcon"]')
