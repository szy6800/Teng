...
# http://fscz.foshan.gov.cn/cfg/index.html
class WjSpider(scrapy.Spider):
    name = 'Finance'
    allowed_domains = ['foshan.gov.cn']
    start_urls = ['http://fscz.foshan.gov.cn/cfg/index.html']
    # start_urls = ['http://fscz.foshan.gov.cn/cfg/index_{}.html'.format(i) for i in range(2,21)]
    ...
    #             p = f"_{p + 1}" if p else ""
    #             url = f"http://cdhrss.chengdu.gov.cn/{cate}{p}.shtml"
    ...
    def parse(self, response):
        ...
        list_url = response.xpath('//*[@class="p42"]/a/@href').getall()
        pub_dates = response.xpath('//*[@class="p43"]/text()').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        for elem in html.xpath(
                '//style|//script|//*[@id="div_div"]|//*[@class="print"]|//*[@id="Canvas"]|//*[@class="bdsharebuttonbox bdshare-button-style1-24"]'):
            elem.getparent().remove(elem)
        content = html.xpath('//*[@class="m15"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="m15"]')
        ...
        compal = re.compile(
            'style=".*?"|https://im.dg.gov.cn/attachment/cmsfile/732158786/0201/201410/daofile/.*?gif|face=".*?"')
        contentHtml = re.sub(compal, '', contentHtml)
        try:
            tables = html.xpath('//*[@class="m15"]//table/@width')[0]
        ...
        attachment_name = response.xpath(
            '//*[(contains(translate(@href, "PDF", "pdf"), ".pdf") or contains(translate(@href, "XLS", "xls"), ".xls") or contains(translate(@href, "DOC", "doc"), ".doc") or contains(translate(@href, "ZIP", "zip"), ".zip") or contains(translate(@href, "ppt", "ppt"), ".ppt") or contains(translate(@href, "RAR", "rar"), ".rar") or contains(translate(@href, "WPS", "wps"), ".wps")) and not(contains(@href , "file://") or contains(@href, "c:\") or contains(@href, "c:\"))]//text() | .//*[contains(translate(@src, "MP4","mp4"), ".mp4") and not(contains(@src, "file://") or contains(@src,"c:\") or contains(@src, "C:\"))]//text()').getall()
