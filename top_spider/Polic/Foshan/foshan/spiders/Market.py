...
class WjSpider(scrapy.Spider):
    name = 'Market'
    allowed_domains = ['fsamr.foshan.gov.cn']
    # start_urls = ['http://fsamr.foshan.gov.cn/zmhd/yjzjjfk/index.html']
    ...
    def start_requests(self):
        for each in index():
            ...
            for p in range(pages):
                p = f"_{p + 1}" if p else ""
                url = f"http://fsamr.foshan.gov.cn/{cate}{p}.html"
                yield scrapy.Request(url=url, callback=self.parse, dont_filter=True)

    def parse(self, response):

        item = {}
        list_url = response.xpath('//*[@class="news_list"]//a/@href').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[@class="content_article"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="content_article"]')
        ...
        try:
            tables = html.xpath('//*[@class="content_article"]//table/@width')[0]
