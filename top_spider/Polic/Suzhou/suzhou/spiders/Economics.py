...
class Wj1Spider(scrapy.Spider):
    name = 'Economics'
    allowed_domains = ['gxj.suzhou.gov.cn']
    start_urls = ['http://gxj.suzhou.gov.cn/szeic/ggl/common_list.shtml']
    # start_urls = ['http://gxj.suzhou.gov.cn/szeic/ggl/common_list_{}.shtml'.format(i) for i in range(2,18)]
    ...
    #             p = f"_{p + 1}" if p else ""
    #             url = f"http://hrss.suzhou.gov.cn/{cate}{p}.shtml"
    ...
    def parse_info(self, response):
        ...
        compal = re.compile('style=".*?"|http://gxj.suzhou.gov.cn/default/images/icon16/.*?gif|face=".*?"')
