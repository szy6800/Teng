...
# https://jiangsu.chinatax.gov.cn/col/col8682/index.html区县通知公告

class WjSpider(scrapy.Spider):
    name = 'Tax'
    allowed_domains = ['jiangsu.chinatax.gov.cn']
    # start_urls = ['http://jiangsu.chinatax.gov.cn/']

    def start_requests(self):
        # 构建url
        # for l1, l2 in zip(range(1, 1350, 45), range(45, 1350, 45)):
        for l1, l2 in zip(range(1, 45, 45), range(45, 90, 45)):
            url = 'https://jiangsu.chinatax.gov.cn/module/web/jpage/dataproxy.jsp?startrecord={}&endrecord={}&perpage=15'.format(
                l1, l2)
            # 构建post请求参数
            data = {
                'col': '1',
