...
class Wj3Spider(scrapy.Spider):
    ...
    start_urls = ['http://czju.suzhou.gov.cn/czj/bsgg/list.shtml']
    # start_urls = ['http://czju.suzhou.gov.cn/czj/bsgg/list_{}.shtml'.format(i) for i in range(2,41)]
    ...
    #             p = f"_{p + 1}" if p else ""
    #
    #             url = f"http://commerce.suzhou.gov.cn/{cate}{p}.shtml"
    ...
    def parse(self, response):
        ...
        list_url = response.xpath("//ul[@class='item lh jt_dott f14']//a/@href").getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        compal = re.compile('style=".*?"|http://czju.suzhou.gov.cn/default/images/icon16/.*?gif|face=".*?"')
