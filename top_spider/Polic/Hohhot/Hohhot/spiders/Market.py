...
import htmlmin
#  http://amr.nmg.gov.cn/zw/tzgg/index_18.html


class GovSpider(scrapy.Spider):
    name = 'Market'
    allowed_domains = ['huhhot.gov.cn']
    start_urls = ['http://amr.nmg.gov.cn/zw/tzgg/index.html']

    def parse(self, response):
        # print(response.text)
        item = {}
        list_url = response.xpath('//*[@class="top_right_con"]//*[@class="hhhei16"]/a/@href').getall()
        #循环遍历
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[contains(@class,"TRS_Edi")]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[contains(@class,"TRS_Edi")]')
        ...
        try:
            tables = html.xpath('//*[contains(@class,"TRS_Edi")]//table/@width')[0]
        ...
        try:
            ...
            pub_date = response.xpath("//*[@name='PubDate']/@content").get()
            timeArray = time.strptime(pub_date, "%Y年%m月%d日 %H:%M")
            timeStamp = int(time.mktime(timeArray)) * 1000
            item['publishTime'] = timeStamp
        # 新闻类型
        item['typeId'] = type_polic(item)
        # 来源
        item['sourceSite'] = '呼和浩特市场监管局'
        # 行业
        item['industry'] = ''
        # 行业id
        item['industryId'] = ''
