...
import htmlmin
#  http://jyj.huhhot.gov.cn/hhhtsjy/xinxigongkai/tzgg/A010101index_3.htm


class GovSpider(scrapy.Spider):
    name = 'Education'
    allowed_domains = ['huhhot.gov.cn']
    start_urls = ['http://jyj.huhhot.gov.cn/hhhtsjy/xinxigongkai/tzgg/A010101index.htm']

    def parse(self, response):
        # print(response.text)
        item = {}
        list_url = response.xpath("//*[contains(@href,'webinfo/20')]/@href").getall()
        pub_date = response.xpath('//*[@width="200"]/text()').getall()
        #循环遍历
        for href,pub_date in zip(list_url,pub_date):
    ...
    def parse_info(self, response):
        ...
        # 标题
        item['title'] = response.xpath('//*[@class="zw-content"]/tbody/tr[3]').get().strip()
        # 城市id 467
        item['cityId'] = 467
        ...
        content = html.xpath('//*[@id="BodyLabel"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@id="BodyLabel"]')
        ...
        try:
            tables = html.xpath('//*[@id="BodyLabel"]//table/@width')[0]
