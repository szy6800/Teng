...
# http://www.fuzhou.gov.cn/zgfzzt/srsj/fzrs/rsdt/tzgg_37425/
class EducationSpider(scrapy.Spider):
    name = 'Human'
    allowed_domains = ['fuzhou.gov.cn']
    start_urls = ['http://www.fuzhou.gov.cn/was5/web/search?channelid=290792&templet=advsch.jsp&sortfield=-docorderpri%2C-docreltime&classsql=chnlid%3D37425&random=0.1602658130481236&prepage=20&page={}'.format(i) for i in range(1,2)] #60
    ...
    def parse_info(self, response):
        ...
        for elem in html.xpath(
                '//style|//script|//*[@id="xl_jd"]|//*[@class="xl-btn-box phone_none"]|//*[@id="qrcode"]'):
            elem.getparent().remove(elem)
        content = html.xpath('//*[@id="detailCon"]//text()|//*[contains(@class,"detailCont")]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@id="detailCon"]|//*[contains(@class,"detailCont")]')
        ...
        try:
            tables = html.xpath('//*[@id="detailCon"]//table/@width')[0]
