...
# http://jyj.fuzhou.gov.cn/zz/zwgk/tzgg_47610/
class EducationSpider(scrapy.Spider):
    name = 'Education'
    allowed_domains = ['fuzhou.gov.cn']
    start_urls = ['http://jyj.fuzhou.gov.cn/was5/web/search?channelid=290792&templet=advsch.jsp&sortfield=-docorderpri%2C-docreltime&classsql=chnlid%3D47610&prepage=15&page={}'.format(i) for i in range(1,2)] #100
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[@id="detailCon"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@id="detailCon"]')
        ...
        for p in p_list:
            ...
            try:
                ...
                pass
        # # 将新的正文标签转成str保存
        contentHtml = etree.tostring(div_data[0], encoding='utf-8').decode().strip()
