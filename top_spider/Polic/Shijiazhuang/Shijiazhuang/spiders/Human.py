...
class MarketsupervisionbureauSpider(scrapy.Spider):
    ...
    start_urls = ['http://rsj.sjz.gov.cn/column.jsp?id=1505457347148&current=1'] #52
    ...
    def parse(self, response):
        # print(response.text)
        item = {}
        list_url = response.xpath('//*[@class="cont_list"]/div/div/a/@href').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        # 标题
        item['title'] = response.xpath("//h1/text()|//*[@name='ArticleTitle']/@content").get().replace('<br>', '').replace('<BR>', '')
        # 城市id 2869
        item['cityId'] = 2869
        ...
        for elem in html.xpath('//style|//script|//*[@class="qrcode200 center"]|//*[@class="print"]|//*[@id="Canvas"]'):
            elem.getparent().remove(elem)
        content = html.xpath('//*[@class="body"]//text()|//*[@id="conN"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        ...
        div_data = html.xpath('//*[@class="body"]|//*[@id="conN"]')
        ...
        try:
            tables = html.xpath('//*[@class="body"]//table/@width')[0]
            item['contentHtml'] = re.sub(tables, '', contentHtml)
        except:
            item['contentHtml'] = re.sub(compal, '', contentHtml)

        pub_date = response.xpath("//*[contains(text(),'时间:')]/text()|//*[contains(text(),'发布时间')]/text()").get()
