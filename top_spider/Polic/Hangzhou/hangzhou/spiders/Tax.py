...
# TODO  杭州市税务局通知公告
#       http://zhejiang.chinatax.gov.cn/col/col17743/index.html?uid=59069&pageNum=1

class WjSpider(scrapy.Spider):
    name = 'Tax'
    allowed_domains = ['zhejiang.chinatax.gov.cn']
    # start_urls = ['http://zhejiang.chinatax.gov.cn/']

    def start_requests(self):
        # 构建url
        for l1, l2 in zip(range(1, 45, 45), range(45, 90, 45)):
            url = 'http://zhejiang.chinatax.gov.cn/module/web/jpage/dataproxy.jsp?startrecord={}&endrecord={}&perpage=15'.format(
                l1, l2)
            # 构建post请求参数
            data = {
                'col': '1',
