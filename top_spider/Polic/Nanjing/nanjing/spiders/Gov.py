...
# http://www.nanjing.gov.cn/zdgk/?id=xxgk_400
class Wj4Spider(scrapy.Spider):
    name = 'Gov'
    allowed_domains = ['njkl.gov.cn']
    start_urls = ['http://www.nanjing.gov.cn/igs/front/search/publish/data/list.html?&index=wzqsearch-v20190124&type=infomation&siteId=3&pageSize=20&orderProperty=DOCRELTIME&pageIndex=2&orderDirection=desc&filter%5BSITEID%5D=3&filter%5BCHANNELID%5D=21985&filter%5BGROUPCAT%5D=406&pageNumber={}'.format(i) for i in range(1,2)]
    ...
    def parse_info(self, response):
        ...
        for elem in html.xpath('//style|//script|//*[@id="nj-share"]|//*[@class="con_f"]|//*[@id="Canvas"]'):
            elem.getparent().remove(elem)
        content = html.xpath("//*[@class='wenZhang']/div//text()")
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath("//*[@class='wenZhang']")
        ...
        try:
            tables = html.xpath('//*[@class="wenZhang"]//table/@width')[0]
