...
class WjSpider(scrapy.Spider):
    name = 'Banking'
    allowed_domains = ['jrj.sh.gov.cn']
    # start_urls = ['http://jrj.sh.gov.cn/ZCJD221/index_{}.html'.format(i) for i in range(1,20)] #20
    start_urls = ['http://jrj.sh.gov.cn/ZCJD221/index.html'] #20

    def parse(self, response):
        item = {}
        list_url = response.xpath('//*[@id="List"]//a/@href').getall()
        titles = response.xpath('//*[@id="List"]//a/@title').getall()
        pub_dates = response.xpath('//div[@class="ejfzggxwright"]').getall()
        # 循环遍历
        for href, pub_date, title in zip(list_url,pub_dates,titles):
            item['url'] = response.urljoin(href.strip())
            item['title'] = title.replace('<br>', '')
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[@class="xxtpqh"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="xxtpqh"]')
        ...
        try:
            tables = html.xpath('//*[@class="xxtpqh"]//table/@width')[0]
