...
# https://xinjiang.chinatax.gov.cn/sszc/zxwj/index_1.htm
class GovSpider(scrapy.Spider):
    name = 'Tax'
    allowed_domains = ['chinatax.gov.cn']
    start_urls = ['https://xinjiang.chinatax.gov.cn/sszc/zxwj/index_{}.htm'.format(i) for i in range(1,2)]

    def parse(self, response):
        # print(response.text)
        item = {}
        list_url = response.xpath('//*[@class="lists"]/li/a/@href').getall()
        pub_dates = response.xpath('//*[@class="lists"]/li/span/text()').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        # 来源
        item['sourceSite'] = '国家税务总局新疆维吾尔自治区税务局'
        # 行业
        item['industry'] = ''
        # 行业id
        item['industryId'] = ''
