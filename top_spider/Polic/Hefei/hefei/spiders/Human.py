...
class WjSpider(scrapy.Spider):
    ...
    start_urls = ['http://rsj.hefei.gov.cn/content/column/6802701?pageIndex={}'.format(i) for i in range(1,2)]
    ...
    def parse(self, response):
        # print(response.text)
        item = {}
        list_url = response.xpath('//*[@class="doc_list list-6802701"]/li/a/@href').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[@class="wzcon j-fontContent clearfix"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="wzcon j-fontContent clearfix"]')
        ...
        compal = re.compile('style=".*?"|http://rsj.hefei.gov.cn/assets/images/files2/.*?gif|face=".*?"')
        contentHtml = re.sub(compal, '', contentHtml)
        try:
            tables = html.xpath('//*[@class="wzcon j-fontContent clearfix"]//table/@width')[0]
