...
class WjSpider(scrapy.Spider):
    name = 'Education'
    allowed_domains = ['hefei.gov.cn']
    # start_urls = ['http://jyj.hefei.gov.cn/jydt/tzgg/index.html']
    start_urls = ['http://jyj.hefei.gov.cn/content/column/6787481?pageIndex={}'.format(i) for i in range(1,2)]
    ...
    def parse(self, response):
        # print(response.text)
        item = {}
        list_url = response.xpath('//*[@class="right date"]/following::a[1]/@href').getall()
        # 循环遍历
    ...
    def parse_info(self, response):
        ...
        content = html.xpath('//*[@class="j-fontContent newscontnet minh300"]//text()')
        sss = ''
        for i in content:
            sss = sss + i
        # 去除
        item['content'] = sss.strip()
        div_data = html.xpath('//*[@class="j-fontContent newscontnet minh300"]')
        ...
        compal = re.compile('style=".*?"|http://czj.hefei.gov.cn/assets/images/files2/.*?gif|face=".*?"')
        contentHtml = re.sub(compal, '', contentHtml)
        try:
            tables = html.xpath('//*[@class="j-fontContent newscontnet minh300"]//table/@width')[0]
