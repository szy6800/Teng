...
def id_sult():
    ...
    sql = f"SELECT * FROM `xian_village`;"
...
def dbz():
    ...
    df2.columns = ['id','url_md5', 'url', 'village', 'districts', 'province', 'provincial_capital', 'lng', 'lat',]# 列表转化为PD格式
    df2 = df2.drop_duplicates(subset="url_md5") # uid去重
    # engine.dispose()
    engine2.dispose() # 关闭连接
    # print(df1)
    db2 = pd.concat([df2, df1], axis=0, sort=False, ignore_index=True) # 合并数据
    # print(db2)
    dbz = db2.drop_duplicates(subset=['url_md5'], keep=False) # 根据uid去重，keep=False不保留重复数据
    # print(dbz)
