...
import datetime
import pymysql
from yushou.items import ZhaobiaoItem
...
def id_sult():
    ...
    sql = f"SELECT uid,uuid,title,link,intro,abs,content,publish_time,purchaser,proxy,create_time,update_time,deleted,province,base,type,items,data_source,end_time,status,serial FROM `caijiqi` WHERE create_time>'{otherStyleTime}';"
...
class XukeSpider(scrapy.Spider):
    ...
    def start_requests(self):
        resultc = id_sult()
        # print(resultc)
        new_url = 'https://www.52zzl.com/huayu/'
